...
for pStor in range(printStor):
    ...
    maxCountsCopy = maxCounts.copy()
    for j in range(maxCountsCopy.__len__()):
        for i in range(maxCountsCopy.__len__()-j-1):
            if (maxCountsCopy[i]<maxCountsCopy[i+1]):
                temp = maxCountsCopy[i]
                maxCountsCopy[i]= maxCountsCopy[i+1]
                maxCountsCopy[i+1]= temp
    #print(maxCountsCopy)


    #ranks the clusters based on the copy

    way2Ranking = [0]*(maxCountsCopy.__len__())
    #set to one from the start to since we are testing cluster numbers from 1-10, not 0-9
    for i in range(maxCountsCopy.__len__()):
        for j in range(maxCounts.__len__()):
            if (maxCountsCopy[i]==maxCounts[j]):
                way2Ranking[i] += kArr[j]
    ...
    print(pStor, "way2ranking", way2Ranking, file = f1)
